 ...
 * Matches the original format: {firstName}_{lastName}_{timestamp}.json
 */
export function exportPatientToJSON(data: PatientData): void {
  const firstName = data.demographics.firstName || 'Patient';
  const lastName = data.demographics.lastName || 'Data';
  const timestamp = Date.now();
  const filename = `${firstName}_${lastName}_${timestamp}.json`;
