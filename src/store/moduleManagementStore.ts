...
const MODULE_PREFERENCES_DB = localforage.createInstance({
  name: 'ayekta-emr-modules',
  storeName: 'preferences',
  description: 'Module preferences and mission data',
});

const PREFERENCES_KEY = 'module-preferences';
const MISSIONS_KEY = 'missions';
const ACTIVE_MISSION_KEY = 'active-mission-id';
...
export const useModuleManagement = create<ModuleManagementState>((set, get) => ({
  ...
  loadPreferences: async () => {
    ...
    try {
      ...
      const preferences = await MODULE_PREFERENCES_DB.getItem<ModulePreferences>(
        PREFERENCES_KEY
      );
      
      // Load missions
      const missions = await MODULE_PREFERENCES_DB.getItem<MissionConfig[]>(
        MISSIONS_KEY
      );
      
      // Load active mission ID
      const activeMissionId = await MODULE_PREFERENCES_DB.getItem<string>(
        ACTIVE_MISSION_KEY
      );
  ...
  savePreferences: async () => {
    ...
    try {
      ...
      await MODULE_PREFERENCES_DB.setItem(PREFERENCES_KEY, preferences);
      
      // Save missions
      await MODULE_PREFERENCES_DB.setItem(MISSIONS_KEY, state.missions);
      
      // Save active mission ID
      if (state.currentMission) {
        await MODULE_PREFERENCES_DB.setItem(
          ACTIVE_MISSION_KEY,
