...
export default function Consent() {
  ...
  // Auto-fill procedure name from Surgical Needs selected procedures
  useEffect(() => {
    const procedures: string[] = [];

    if (surgicalNeeds.opInguinalHerniaL) procedures.push('Inguinal Hernia Repair (Left)');
    if (surgicalNeeds.opInguinalHerniaR) procedures.push('Inguinal Hernia Repair (Right)');
    if (surgicalNeeds.opInguinalHerniaBilateral) procedures.push('Inguinal Hernia Repair (Bilateral)');
    if (surgicalNeeds.opVentralUmbilicalHernia) procedures.push('Ventral/Umbilical Hernia Repair');
    if (surgicalNeeds.opHysterectomy) procedures.push('Hysterectomy');
    if (surgicalNeeds.opProstatectomy) procedures.push('Prostatectomy');
    if (surgicalNeeds.opHydrocelectomyL) procedures.push('Hydrocelectomy (Left)');
    if (surgicalNeeds.opHydrocelectomyR) procedures.push('Hydrocelectomy (Right)');
    if (surgicalNeeds.opHydrocelectomyBilateral) procedures.push('Hydrocelectomy (Bilateral)');
    if (surgicalNeeds.opMassExcision && surgicalNeeds.massExcisionLocation) {
      procedures.push(`Mass Excision (${surgicalNeeds.massExcisionLocation})`);
    } else if (surgicalNeeds.opMassExcision) {
      procedures.push('Mass Excision');
    }
    if (surgicalNeeds.operationOther) procedures.push(surgicalNeeds.operationOther);

    if (procedures.length > 0 && !consent.procedureName) {
      updateConsent({ procedureName: procedures.join(', ') });
    }
  }, [surgicalNeeds, consent.procedureName, updateConsent]);

  // Auto-fill performing surgeon from Triage attending surgeon
  useEffect(() => {
    if (attendingSurgeon && !consent.performingSurgeon) {
      updateConsent({ performingSurgeon: attendingSurgeon });
    }
  }, [attendingSurgeon, consent.performingSurgeon, updateConsent]);
      ...
      <div className="bg-white rounded-lg shadow-sm border border-ayekta-border p-6 space-y-6">
        ...
        {consent.procedureName && (
          <section className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <h3 className="text-lg font-semibold mb-2 text-blue-900">Consent Summary</h3>
            <p className="text-sm text-blue-800">
              <strong>Procedure:</strong> {consent.procedureName}
              {consent.performingSurgeon && <> | <strong>Surgeon:</strong> {consent.performingSurgeon}</>}
              {consent.plannedDate && <> | <strong>Date:</strong> {consent.plannedDate}</>}
            </p>
          </section>
        )}
        ...
        <section>
          ...
          <div className="space-y-4">
            ...
            {consent.interpreterUsed && (
              <div className="bg-gray-50 border border-ayekta-border rounded p-4 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-1">Interpreter Name / Service</label>
                    <input
                      type="text"
                    ...
                    {(['in-person', 'phone', 'video', 'family-member'] as const).map((type) => (
                      <label key={type} className="flex items-center gap-2 cursor-pointer">
                        <input
                          ...
                          className="w-4 h-4 text-ayekta-orange border-ayekta-border focus:ring-ayekta-orange"
                        />
                        <span className="text-sm capitalize">{type === 'in-person' ? 'In-Person' : type === 'family-member' ? 'Family Member' : type.charAt(0).toUpperCase() + type.slice(1)}</span>
                      </label>
                    ))}
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="p-4 border border-ayekta-border rounded">
              <SignatureField
                ...
                providerName={consent.witnessName}
                signatureDate={consent.witnessSignatureDate}
                onProviderNameChange={(value) => handleChange('witnessName', value)}
                onSignatureDateChange={(value) => handleChange('witnessSignatureDate', value)}
            ...
            <div className={`p-4 border border-ayekta-border rounded transition-opacity${!allCheckboxesChecked ? ' opacity-50 pointer-events-none select-none' : ''}`}>
              <SignatureField
                ...
                signatureDate={consent.providerSignatureDate}
